...
<div class="ag-stepper">
  <span class="ag-step active">① Register</span>
...
st.write(
    "Register yourself before analyzing crop diseases."
)
...
if submit:
    ...
    if not name.strip():
        ...
        with st.spinner("Registering Farmer..."):

            response = APIClient.register_farmer(
                name,
                location
            )
        ...
        if "id" in response:
            ...
            st.info(
                f"Farmer ID : {response['id']}"
            )

            st.session_state["farmer_id"] = response["id"]
